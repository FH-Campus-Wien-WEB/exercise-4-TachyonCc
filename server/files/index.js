  ...
  fetch(`/movies/${imdbID}`, { method: 'PUT' })
    .then(response => {
        if (response.status === 201) {
            const resultElement = document.getElementById("search-result-" + imdbID);
            if (resultElement) {
  ...
  function renderUserGreeting() {
    ...
    if (currentSession) {
        ...
        greetingElement.textContent =
            ...
            " " +
            currentSession.lastName +
            ", du hast dich am " +
            loginDate.toLocaleDateString("de-AT") +
            " um " +
            loginDate.toLocaleTimeString("de-AT", {
                hour: "2-digit",
                minute: "2-digit"
            }) +
            " angemeldet.";
